...
- *What are the limitations of an automated harmonisation pipeline?* \ Literature on existing harmonisation apporaches will be reviewed and known issues will be noted.
...
- *What are the existing DEMs at different scales (regional, national, and global)?* \ As described in @data-discovery, the regional and national datasets will be findable from national geoportals. Further data discovery is needed for global and international (e.g. pan-European) elevation models; supervisory team will be consulted as they have experience with them. The resolution, spatial extent and notable individual characteristics of the datasets will be documented.
- *What are the issues with cross-border data?* \ As described in @qa, border regions will be sampled and reviewed; found issues, if different from the issues found in literature, these will be documented.
...
As described in @sec:theoretical-approach, literature review needs to be conducted to answer most of the subquestions. The literature will be collected on the following themes:
...
- *Comparison to existing DEMs.* The best candidate for this step is Copernicus DEM published by the European Space Agency. While the datasets will have spatial (mainly resolution — Copernicus DEM is freely available at 30m resolution while expected resolution of Rhine DEM is 5m or finer) and temporal (dates of acquisition) discrepancies, this should still provide some overview of the pipeline's performance across different areas of the catchment area.
